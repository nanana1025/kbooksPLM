...
{
    public partial class usrReceiptPartTree : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public usrReceiptPartTree()
        {
            ...
            _dt.Columns.Add(new DataColumn("COMPARE_CNT", typeof(string)));
        ...
        private void tlPart_FocusedNodeChanged(object sender, DevExpress.XtraTreeList.FocusedNodeChangedEventArgs e)
        {
            ...
            if (_FocusedNode != null)
            {
                ...
                _id = ConvertUtil.ToInt64(_FocusedNode["P_PART_ID"]);
        ...
        private void setNodeAllowEdit()
        {
            ...
            tlcComparePart.OptionsColumn.AllowEdit = allowCompare;
            tlcCompareCnt.OptionsColumn.AllowEdit = allowCompare;
        }

        public void getSummaryValue(ref int cost, ref int adjustCost, ref int totalCost )
        {
            cost = ConvertUtil.ToInt32(tlPart.GetSummaryValue(tlPurchaseCost));
            adjustCost = ConvertUtil.ToInt32(tlPart.GetSummaryValue(tlAdjustCost));
            totalCost = ConvertUtil.ToInt32(tlPart.GetSummaryValue(tlTotalCost));
        }
                ...
                if (drPurchase.Length > 0)
                {
                    foreach (DataRow row in drPurchase)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                dr["COMPARE"] = "0";
                                dr["COMPARE_CNT"] = row["PART_CNT"];
                                dr["EXAMINE_PART_CNT"] = row["PART_CNT"];
                                dr["PURCHASE_COST"] = 0;
        ...
        public JArray makeExamComplete()
        {
            ...
            foreach (DataRow row in _dt.Rows)
            {
                ...
                jdata.Add("DIFF_CNT", ConvertUtil.ToString(row["COMPARE_CNT"]));
        ...
        public void updateCnt(int cnt)
        {
            ...
            tlPart.CellValueChanged -= tlPart_CellValueChanged;
            _FocusedNode["RECEIPT_PART_CNT"] = cnt;
            long currentTotalCost = ConvertUtil.ToInt64(_FocusedNode["RECEIPT_PRICE"]) * ConvertUtil.ToInt64(cnt);
            _FocusedNode["TOTAL_COST"] = _FocusedNode["ADJUST_COST"] = _FocusedNode["RECEIPT_COST"] = currentTotalCost;
            tlPart.CellValueChanged += tlPart_CellValueChanged;
            _totalCost = _totalCost - _componentTotalCost + currentTotalCost;
        ...
        private void tlPart_NodeCellStyle(object sender, DevExpress.XtraTreeList.GetCustomNodeCellStyleEventArgs e)
        {
            if (e != null && e.Node != null && e.Node["PART_ID"] != null && e.Node != tlPart.FocusedNode)
            {
                ...
                if (e.Column.FieldName.Equals("RECEIPT_COST") || e.Column.FieldName.Equals("PURCHASE_COST") || e.Column.FieldName.Equals("ADJUST_COST") || e.Column.FieldName.Equals("TOTAL_COST"))
                {
                    long value = ConvertUtil.ToInt64(e.Node[e.Column.FieldName]);
