...
namespace WareHousingMaster.view.consigned
{
    public partial class DlgImportConsignedReceiptByModel : DevExpress.XtraEditors.XtraForm
    {
            ...
            int cnt)
        {
            ...
            _listModelKeyword = new List<string>();
        ...
        private bool AddReceiptByExcel1(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                foreach(string st in delimiter2)
                {
                ...
                if (isFullSpec)
                {
                    ...
                    {
                        modelCapslock = model.ToUpper();
                        foreach (string word in _listModelKeyword)
                        {
                            if (modelCapslock.Contains(word))
                            {
                                keyword = word;
                                break;
                            }
                        ...
                        {
                            cpu = getCpu(arrModelPart[1], ref gen, ref code);

                            DataRow[] rows = _dtCompanyModel.Select($"MODEL_NM LIKE '%{keyword}%' AND MODEL_NM LIKE '%{code}'");

                            if (rows.Length > 0)
                            {
                                DataRow modelRow = rows[0];

                                modelNm = ConvertUtil.ToString(modelRow["MODEL_NM"]);
                                deviceType = ConvertUtil.ToInt32(modelRow["PC_TYPE"]);
                                dr["MODEL_NM"] = modelNm;
                                dr["TYPE"] = deviceType;

                                memTypeCheck = ConvertUtil.ToInt32(modelRow["MEM_TYPE"]);
                                memSlotCnt = ConvertUtil.ToInt32(modelRow["MEM_SLOT"]);
                                ...
                                jdata.Add("MODEL_LIST_ID", ConvertUtil.ToInt64(modelRow["MODEL_LIST_ID"]));
                                ...
                                dr["MODEL_ID"] = rows[0]["MODEL_LIST_ID"];
                                dr["MODEL_ID_NM"] = rows[0]["MODEL_NM"];
                    ...
                    else if (arrModelPart.Length > 0)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(keyword))
                        {
                            ...
                            DataRow[] rows = _dtCompanyModel.Select($"MODEL_NM LKIE '%{keyword}%'");

                            if (rows.Length > 1)
                            {
                                dr["CHECK"] = false;
                                dr["ERROR"] = "일치하는 모델 복수. 상세정보 필요";
                            }
                            else if(rows.Length == 1)
                            {
                                ...
                                JObject jdataModel = new JObject();

                                jdataModel.Add("ID", id);
                                jdataModel.Add("MODEL_LIST_ID", ConvertUtil.ToInt64(modelRow["MODEL_LIST_ID"]));

                                jArrayModel.Add(jdataModel);

                                dr["MODEL_ID"] = modelRow["MODEL_LIST_ID"];
                                dr["MODEL_ID_NM"] = modelRow["MODEL_NM"];
                ...
                {
                    DataRow[] rows = _dtCompanyModel.Select($"MODEL_NM = '{model}'");
            ...
            {
                jobj.Remove("DATA");
                jobj.Remove("COMPANY_ID");
                jobj.Remove("USER_ID");
                jobj.Add("DATA", jArrayModel); 
                jobj.Add("COMPANY_ID", _companyId);

                JObject jResult = new JObject();

                if (DBConsigned.getReceiptModel(jobj, ref jResult))
                {
                    JArray jArrayResult = JArray.Parse(jResult["DATA"].ToString());
                    ...
                    tlResult.Invoke(new Action(() => tlResult.Nodes.Add(new object[] { ++_errCount, $"모델구성... ", (int)EnumInfoCode.Info })));

                    foreach (JObject obj in jArrayResult.Children<JObject>())
                    { 
                        nId = ConvertUtil.ToInt32(obj["ID"]);
                        DataRow[] rows = _dtReceipt.Select($"ID = {nId}");
                        ...
                        repModelNm = ConvertUtil.ToString(obj["MODEL_NM"]);
                        componentCnt = ConvertUtil.ToInt32(obj["COMPONENT_CNT"]);

                        if (consignedType == 2)
                        {
                            if (_dicConsignedType.ContainsKey(nId))
                            {
                                List<long> listConsignedType = _dicConsignedType[nId];
                            ...
                            else
                            {
                                ...
                                listConsignedType.Add(componentId);
                                _dicConsignedType.Add(nId, listConsignedType);
                            }
                        }

                        if (!_dicComponentNm.ContainsKey(componentId))
                            _dicComponentNm.Add(componentId, repModelNm);


                        if (_dicConsignedComponent.ContainsKey(nId))
                        {
                            Dictionary<string, Dictionary<long, int>> dicComponent = _dicConsignedComponent[nId];

                            if(dicComponent.ContainsKey(componentCd))
                            {
                                Dictionary<long, int> dicComp = dicComponent[componentCd];
                                dicComp.Add(componentId, componentCnt);
                            }
                            else
                            {
                                Dictionary<long, int> dicComp = new Dictionary<long, int>();
                                dicComp.Add(componentId, componentCnt);
                                dicComponent.Add(componentCd, dicComp);
                            }
                        }
                        else
                        {
        ...
        private string getCpu(string str, ref string gen, ref string code)
        {
            Char[] delimiter = { '(', ')', '[', ']', '/', '+', ' ', '-' };
            str = str.ToUpper();
            string[] arrStr = str.Trim().Split(delimiter);
        ...
        private void getCompanyModelList()
        {
            ...
            if (DBConsigned.getCompanyModelList(jobj, ref jResult))
            {
                if (ConvertUtil.ToBoolean(jResult["DATA_EXIST"]))
                {
                    ...
                    {
                        keyword = ConvertUtil.ToString(jData["KEYWORD"]).ToUpper();
                        ...
                        if (!_listModelKeyword.Contains(keyword))
                            _listModelKeyword.Add(keyword);
                    }
                }
            }
