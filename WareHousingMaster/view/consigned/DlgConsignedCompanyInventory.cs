...
{
    public partial class DlgConsignedCompanyInventory : DevExpress.XtraEditors.XtraForm
    {
        ...
        public List<long> _listComponentId { private set; get; }


        public DlgConsignedCompanyInventory(long companyId)
        {
            InitializeComponent();
            ...
            _listComponentId = new List<long>();
        }
        ...
        private void lcgInventory_CustomButtonClick(object sender, DevExpress.XtraBars.Docking2010.BaseButtonEventArgs e)
        {
            ...
            {
                using (DlgCompanyPartStatistice dlgCompanyPartStatistice = new DlgCompanyPartStatistice(_companyId))
                {
        ...
        private void setComponentInfo(bool isBulk, int type)
        {
            ...
            else
            {
                ...
                if (type == 1)
                {
                    ...
                    _drvCompanyInventory["SPARE_CNT"] = _drvCompanyInventory["SPAREL_CNT_L"];
        ...
        private void sbSave_Click(object sender, EventArgs e)
        {
            ...
            if (Dangol.MessageYN($"선택하신 부품을 모델에 추가하시겠습니까?") == DialogResult.Yes)
            {
                JObject jResult = new JObject();
                JObject jobj = new JObject();
                long componentId;
                ...
                {
                    componentId = ConvertUtil.ToInt64(row["COMPONENT_ID"]);
                    if (!_listComponentId.Contains(componentId))
                    {
                        _listComponentId.Add(componentId);
                    }
