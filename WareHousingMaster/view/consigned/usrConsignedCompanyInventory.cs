...
{
    public partial class usrConsignedCompanyInventory : DevExpress.XtraEditors.XtraForm
    {

        DataTable _dtCompany;
        DataTable _dtCompanyInventory;
        DataTable _dtCompanyPreInventory;
        ...
        List<string> _listPreComponentCd;


        public usrConsignedCompanyInventory()
        {
            InitializeComponent();
            ...
            _dtCompanyInventory.Columns.Add(new DataColumn("PRE", typeof(int)));

            _dtCompanyInventory.Columns.Add(new DataColumn("CHECK", typeof(bool)));


            _dtCompanyPreInventory = new DataTable();

            _dtCompanyPreInventory.Columns.Add(new DataColumn("ROOT_COMPONENT_ID", typeof(long)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("COMPONENT_ID", typeof(long)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("COMPANY_ID", typeof(long)));
            
            _dtCompanyPreInventory.Columns.Add(new DataColumn("TOTAL_CNT_A", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("GOOD_CNT_A", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("RELEASE_CNT_A", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("FAULT_CNT_A", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("SPARE_CNT_A", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("LOCK_CNT_A", typeof(int)));

            _dtCompanyPreInventory.Columns.Add(new DataColumn("TOTAL_CNT_L", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("GOOD_CNT_L", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("RELEASE_CNT_L", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("FAULT_CNT_L", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("SPARE_CNT_L", typeof(int)));
            _dtCompanyPreInventory.Columns.Add(new DataColumn("LOCK_CNT_L", typeof(int)));          
            ...
            _listPreComponentCd = new List<string>(new[]{"MEM", "CAB" });
        }
        ...
        private void usrConsignedAdjustment_Load(object sender, EventArgs e)
        {
            ...
            if (ProjectInfo._userType.Equals("E"))
            {
                ...
                for (int i = 1; i < lcgInventory.CustomHeaderButtons.Count; i++)
        ...
        {
            string componentCd = ConvertUtil.ToString(jData["COMPONENT_CD"]);
            long componentId = ConvertUtil.ToInt64(jData["COMPONENT_ID"]);
            ...
            if (_listPreComponentCd.Contains(componentCd))
            {
                DataRow[] rows = _dtCompanyPreInventory.Select($"COMPONENT_ID = {componentId}");

                if (rows.Length > 0)
                    dr1["PRE"] = 1;

                //rows = _dtCompanyPreInventory.Select($"ROOT_COMPONENT_ID = {componentId}");
                ...
                //    if (consignedType == 1)
                //    {
                //        dr1["TOTAL_CNT"] = ConvertUtil.ToInt32(dr1["TOTAL_CNT_L"]) - ConvertUtil.ToInt32(row["RELEASE_CNT_L"]);
                //        dr1["GOOD_CNT"] = ConvertUtil.ToInt32(dr1["GOOD_CNT_L"]) - ConvertUtil.ToInt32(row["RELEASE_CNT_L"]);
                //        dr1["RELEASE_CNT"] = ConvertUtil.ToInt32(dr1["RELEASE_CNT_L"]) + ConvertUtil.ToInt32(row["RELEASE_CNT_L"]);
                ...
                //    {
                //        dr1["TOTAL_CNT"] = ConvertUtil.ToInt32(dr1["TOTAL_CNT_A"]) - ConvertUtil.ToInt32(row["RELEASE_CNT_A"]);
                //        dr1["GOOD_CNT"] = ConvertUtil.ToInt32(dr1["GOOD_CNT_A"]) - ConvertUtil.ToInt32(row["RELEASE_CNT_A"]);
                //        dr1["RELEASE_CNT"] = ConvertUtil.ToInt32(dr1["RELEASE_CNT_A"]) + ConvertUtil.ToInt32(row["RELEASE_CNT_A"]);
                ...
                //    {
                //        dr1["TOTAL_CNT"] = ConvertUtil.ToInt32(dr1["TOTAL_CNT_A"]) + ConvertUtil.ToInt32(dr1["TOTAL_CNT_L"]) - (ConvertUtil.ToInt32(row["RELEASE_CNT_A"]) + ConvertUtil.ToInt32(row["RELEASE_CNT_L"]));
                //        dr1["GOOD_CNT"] = ConvertUtil.ToInt32(dr1["GOOD_CNT_A"]) + ConvertUtil.ToInt32(dr1["GOOD_CNT_L"]) - (ConvertUtil.ToInt32(row["RELEASE_CNT_A"]) + ConvertUtil.ToInt32(row["RELEASE_CNT_L"]));
                //        dr1["RELEASE_CNT"] = ConvertUtil.ToInt32(dr1["RELEASE_CNT_A"]) + ConvertUtil.ToInt32(dr1["RELEASE_CNT_L"]) + (ConvertUtil.ToInt32(row["RELEASE_CNT_A"]) + ConvertUtil.ToInt32(row["RELEASE_CNT_L"]));
        ...
        private bool getCompanyInventoryList(long companyId)
        {
            ...
            if (getCompanyPreInventoryList(companyId))
            {
        ...
        private bool getCompanyPreInventoryList(long companyId)
        {
            JObject jResult = new JObject();
            JObject jObj = new JObject();

            _dtCompanyPreInventory.Clear();

            jObj.Add("COMPANY_ID", companyId);

            if (DBConsigned.getCompanyPreInventoryList(jObj, ref jResult))
            {
                if (ConvertUtil.ToBoolean(jResult["DATA_EXIST"]))
                {
                    ...
                    foreach (JObject jData in jArray.Children<JObject>())
                    {
                        DataRow dr1 = _dtCompanyPreInventory.NewRow();
                        ...
                        dr1["ROOT_COMPONENT_ID"] = jData["ROOT_COMPONENT_ID"];
                        dr1["COMPONENT_ID"] = jData["COMPONENT_ID"];

                        _dtCompanyPreInventory.Rows.Add(dr1);
                    }
                }
        ...
        private void lcgInventory_CustomButtonClick(object sender, DevExpress.XtraBars.Docking2010.BaseButtonEventArgs e)
        {
            ...
            else if (e.Button.Properties.Tag.Equals(5))
            {
                ...
                using (DlgCompanyPartReturnStatistice dlgCompanyPartReturnStatistice = new DlgCompanyPartReturnStatistice(companyId))
                {
                    if (dlgCompanyPartReturnStatistice.ShowDialog(this) == DialogResult.OK)
                    {
            ...
            else if (e.Button.Properties.Tag.Equals(6))
            {
                ...
                using (DlgCompanyPartWarehousingStatistice dlgCompanyPartWarehousingStatistice = new DlgCompanyPartWarehousingStatistice(companyId))
                {
                    if (dlgCompanyPartWarehousingStatistice.ShowDialog(this) == DialogResult.OK)
                    {
            ...
            else if (e.Button.Properties.Tag.Equals(7))
            {
                using (SaveFileDialog form = new SaveFileDialog())
                {
                    ...
                    if (form.ShowDialog(this) == DialogResult.OK)
                    {
                        ...
                        gcCompanyInventory.ExportToXlsx(form.FileName, options);
        ...
        private void gvCompanyInventory_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            ...
            else if (e.RowHandle >= 0 && e.Column.FieldName == "TOTAL_CNT")
            {
                int value = ConvertUtil.ToInt32(View.GetRowCellValue(e.RowHandle, View.Columns["TOTAL_CNT"]));

                if (value < 0)
                    e.Appearance.BackColor = System.Drawing.Color.Orange;
            }
            else if (e.RowHandle >= 0 && e.Column.FieldName == "GOOD_CNT")
            {
                int value = ConvertUtil.ToInt32(View.GetRowCellValue(e.RowHandle, View.Columns["GOOD_CNT"]));

                if (value < 0)
                    e.Appearance.BackColor = System.Drawing.Color.Orange;
            }
            else if (e.RowHandle >= 0 && e.Column.FieldName == "MODEL_NM")
            {
                int value = ConvertUtil.ToInt32(View.GetRowCellValue(e.RowHandle, View.Columns["PRE"]));
        ...
        {
            if(lcgInventory.CustomHeaderButtons[5].Properties.Checked)
            {
                bsCompanyInventory.Filter = "VISIBLE_YN < 2";
                gcVisibleYn.Visible = true;
            }
