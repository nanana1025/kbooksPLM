...
{
    public partial class usrRegisterPurchaseInfo : DevExpress.XtraEditors.XtraUserControl
    {
        public DataTable _dt { get; private set; }
        public BindingSource _bs { get; private set; }
        public DataRowView _currentRow { get; set; }

        Dictionary<string, string> _dicPurchaseInfo;
        DataTable _dtPurchaseInfo;
        int _price;
        ...
        public usrRegisterPurchaseInfo()
        {
            InitializeComponent();
            ...
            _dicPurchaseInfo = new Dictionary<string, string>();
        ...
        public void setInitLoad(Dictionary<string, string> dicPurchaseInfo, DataTable dtPurchaseInfo, int price)
        {
            _dicPurchaseInfo = dicPurchaseInfo;
            _dtPurchaseInfo = dtPurchaseInfo;
            _price = price;
            setInfoBox();
            setinitialize();
        }
        ...
        public void setinitialize()
        {
            ...
            _bs.DataSource = _dtPurchaseInfo;
            gcList.DataSource = _bs;

            tePurchaseNm.Text = _dicPurchaseInfo["PURCHNM"];
            tePurchaseCd.Text = _dicPurchaseInfo["PURCHCD"];
            sePriority.EditValue = ConvertUtil.ToInt32(_dicPurchaseInfo["PRIORITY"]);
        }
        ...
        private void tePurchaseNm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                int shopCd = 1;
                string purchaseNm = tePurchaseNm.Text.Trim();

                using (dlgPurchaseList purchaseList = new dlgPurchaseList(purchaseNm, shopCd))
                {
                    ...
                    if (purchaseList.ShowDialog(this) == DialogResult.OK)
                    {
                        _dicPurchaseInfo["PURCHCD"] = tePurchaseCd.Text = ConvertUtil.ToString(purchaseList._drv["PURCHCD"]);
                        _dicPurchaseInfo["PURCHNM"] = tePurchaseNm.Text = ConvertUtil.ToString(purchaseList._drv["PURCHNM"]);
                        sePriority.Focus();
                    }
                }
            }
        ...
        private void tePurchaseCd_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                JObject jResult = new JObject();
                string query = $"SELECT PURCHCD, PURCHNM FROM HMA02 WHERE PURCHCD = {purchaseCd}";

                if (DBConnect.getRow(query, ref jResult))
                {
                    if (Convert.ToBoolean(jResult["EXIST"]))
                    {
                        _dicPurchaseInfo["PURCHCD"] = tePurchaseCd.Text = ConvertUtil.ToString(jResult["PURCHCD"]);
                        _dicPurchaseInfo["PURCHNM"] = tePurchaseNm.Text = ConvertUtil.ToString(jResult["PURCHNM"]);
                        sePriority.Focus();
                    }
                    else
                    {
        ...
        private void gvList_CellValueChanging(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (_currentRow != null)
            {
                //spinedi textEditor = (Edit)sender;

                //if (e.Column.FieldName == "PURCHASE_RATIO")
                //{
                //    int ratio = ConvertUtil.ToInt32(_currentRow["PURCHASE_RATIO"]);
                //    double dPrice = (double)_price * (double)ratio / 100.0;
                //    _currentRow["PRICE"] = ConvertUtil.ToInt32(dPrice);
                //}
                //else if (e.Column.FieldName == "PRICE")
                //{
                //    int price = ConvertUtil.ToInt32(_currentRow["PURCHASE_RATIO"]);
                //    double dRatio = (double)price / (double)_price * 100.0;
                //    _currentRow["PURCHASE_RATIO"] = ConvertUtil.ToInt32(dRatio);
                //}
            }
        }

        private void risePrice_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            string sPrice = ConvertUtil.ToString(e.NewValue);
            _currentRow.BeginEdit();
            if (string.IsNullOrEmpty(sPrice))
            {
                _currentRow["PURCHASE_RATIO"] = DBNull.Value;
            }
            else
            {
                if (_price == 0)
                    _currentRow["PURCHASE_RATIO"] = 0;
                else
                {
                    int price = ConvertUtil.ToInt32(e.NewValue);
                    double dRatio = (double)price / (double)_price * 100.0;
        ...
        private void riseRatio_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            string sRatio = ConvertUtil.ToString(e.NewValue);
            _currentRow.BeginEdit();
            ...
            else
            {
                ...
                {
                    int ratio = ConvertUtil.ToInt32(e.NewValue);
                    double dPrice = (double)_price * (double)ratio / 100.0;
                    _currentRow["PRICE"] = ConvertUtil.ToInt32(dPrice);
                }
            }
        ...
        private void sePriority_EditValueChanged(object sender, EventArgs e)
        {
            _dicPurchaseInfo["PRIORITY"] = sePriority.Text;
        }
    }
}
