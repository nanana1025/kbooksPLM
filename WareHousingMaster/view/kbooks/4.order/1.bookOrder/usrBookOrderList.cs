...
namespace WareHousingMaster.view.kbooks.search.booksearch
{
    public partial class usrBookOrderList : DevExpress.XtraEditors.XtraUserControl
    {
                    ...
                    if (bookSearch.ShowDialog(this) == DialogResult.OK)
                    {
                            ...
                            if (DBConnect.getRequest(jobj, ref jResult, url))
                            {
                                if (Convert.ToBoolean(jResult["EXIST"]))
                                {
                                    ...
                                    foreach (JObject obj in jArray.Children<JObject>())
                                    {
                                        ...
                                        if (index == 1)
                                        {
                                            firstPurchCd = _selectedPurchCd = purchCd;
                                            index++;
                                        }
                                    }
                        ...
                        else
                        {
                            ...
                            if (dtPurchcd.Rows.Count > 0)
                            {
                                ...
                                _currentRow.BeginEdit();
                                _currentRow["PURCHCD"] = _selectedPurchCd = purchCd;
                                _currentRow.EndEdit();
                ...
                if (Convert.ToBoolean(jResult["EXIST"]))
                {
                    ...
                    if (!_dicLookUpEdit.ContainsKey(bookCd))
                    {
                        ...
                        if (DBConnect.getRequest(jobj, ref jResult, url))
                        {
                            if (Convert.ToBoolean(jResult["EXIST"]))
                            {
                                ...
                                getBookOrdInfo(bookCd, ConvertUtil.ToInt32(row["PURCHCD"]), row);
                    ...
                    else
                    {
                        ...
                        if (dtPurchcd.Rows.Count > 0)
                        {
                            ...
                            row["PURCHCD"] = _selectedPurchCd = prePurchCd > 0 ? prePurchCd : purchCd;
        ...
        private void showEditor(string col)
        {
            if (col == "PURCHCD")
            {

            }
            else if (col == "ORDER_RATIO")
            {
                ...
                if (_dicOrderRatioTable.ContainsKey(key))
                {
                    rileOrderRatio.DataSource = _dicOrderRatioTable[key];
                    int value = ConvertUtil.ToInt32(_dicOrderRatioTable[key].Rows[0]["KEY"]);
                    _currentRow["ORDER_RATIO"] = value;
                }
                else
                    rileOrderRatio.DataSource = null;
            }
        }
        ...
        private void getBookOrdInfo(long bookCd, int purchCd, DataRow row)
        {
            if (bookCd > 0 && purchCd > 0)
            {
                JObject jResult = new JObject();
                ...
                if (DBConnect.getRequest(jobj, ref jResult, url))
                {
                    if (row != null)
                    {
                        ...
                        row.BeginEdit();
                        cnt = ConvertUtil.ToInt32(jResult["INP_CNT"]);
                        if (cnt != 0) row["INP_CNT"] = cnt;
                        else row["INP_CNT"] = DBNull.Value;

                        cnt = ConvertUtil.ToInt32(jResult["RET_CNT"]);
                        if (cnt != 0) row["RETURN_CNT"] = cnt;
                        else row["RETURN_CNT"] = DBNull.Value;

                        cnt = ConvertUtil.ToInt32(jResult["ORD_CNT"]);
                        if (cnt != 0) row["ORD_CNT"] = cnt;
                        else row["ORD_CNT"] = DBNull.Value;

                        cnt = ConvertUtil.ToInt32(jResult["ESTI_CNT"]);
                        if (cnt != 0) row["ESTI_CNT"] = cnt;
                        else row["ESTI_CNT"] = DBNull.Value;
                        row.EndEdit();
                    }
                }
                else
        ...
        private void rileOrderRatio_EditValueChanged(object sender, EventArgs e)
        {
            ...
            long bookCd = ConvertUtil.ToInt64(_currentRow["BOOKCD"]);
            int purchCd = ConvertUtil.ToInt32(_currentRow["PURCHCD"]);
            int orderRatio = ConvertUtil.ToInt32(editor.EditValue);
        ...
        private void rilePurchCd_Popup(object sender, EventArgs e)
        {
