...
{
    public partial class usrBookSearchList : DevExpress.XtraEditors.XtraUserControl
    {
                ...
                if (DBConnect.getRequest(jobj, ref jResult, url))
                {
                    if (Convert.ToBoolean(jResult["EXIST"]))
                    {
                        ...
                        foreach (JObject obj in jArray.Children<JObject>())
                        {
                            ...
                            dr["PUBSHCD"] = ConvertUtil.ToString(obj["PUBSHCD"]);
                            dr["PUBSHNM"] = ConvertUtil.ToString(obj["PUBSHNM"]);
                            dr["AUTHOR1"] = ConvertUtil.ToString(obj["AUTHORNM"]);
