...
namespace WareHousingMaster.view.kbooks.search.common
{
    ...
    public partial class dlgGetAddress : DevExpress.XtraEditors.XtraForm
    {

        #region "FORM_LOAD"
        private int intLFrmLoading = 0; // 폼로드여부
        public dlgGetAddress()
        {
            InitializeComponent();
        }

        private void dlgGetAddress_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                webBrowser1.Navigate($"{ProjectInfo._AddressUrl}/exceptionWeb.do?content=address");
        ...
        private string lfn_html()
        {
            ...
            strhtml += " data.zipcode             ,data.address           ,data.addressEnglish         , data.addressType         ,data.userSelectedType";
