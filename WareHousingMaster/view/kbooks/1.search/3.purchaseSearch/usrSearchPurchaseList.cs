...
{
    public partial class usrSearchPurchaseList : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void getList(JObject jobj)
        {
            ...
            string url = "/search/getPurchaseList.json";
                ...
                if (DBConnect.getRequest(jobj, ref jResult, url))
                {
                    if (Convert.ToBoolean(jResult["EXIST"]))
                    {
                        ...
                        foreach (JObject obj in jArray.Children<JObject>())
                        {
                            ...
                            if(!string.IsNullOrWhiteSpace(ConvertUtil.ToString(obj["TEL1"])))
                                dr["TEL"] = $"{obj["TEL1"]}-{obj["TEL2"]}-{obj["TEL3"]}";
                            if (!string.IsNullOrWhiteSpace(ConvertUtil.ToString(obj["FAX1"])))
                                dr["FAX"] = $"{obj["FAX1"]}-{obj["FAX2"]}-{obj["FAX3"]}";
