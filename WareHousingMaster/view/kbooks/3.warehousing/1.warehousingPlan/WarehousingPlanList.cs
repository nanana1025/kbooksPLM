...
{
    public partial class WarehousingPlanList : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        LookUpEdit _lookupRate;
        PopupLookUpEditForm popup;
        ...
        private void setInfoBox()
        {
            ...
            Util.insertRow(dtCondition, 1, "1");
            Util.insertRow(dtCondition, 2, "2");
            Util.insertRow(dtCondition, 3, "3");
            Util.insertRow(dtCondition, 4, "4");
            Util.insertRow(dtCondition, 11, "11");
            Util.insertRow(dtCondition, 12, "12");
            Util.insertRow(dtCondition, 13, "13");
            Util.insertRow(dtCondition, 14, "14");
        ...
        public void setCondition(JObject obj)
        {
            ...
            _orderNotice = ConvertUtil.ToString(obj["ORDER_NOTICE"]);
            ...
            //_orderNotice = ConvertUtil.ToString(obj["ORDER_CONTIDION_STRING"]);
        ...
        public void setTableInitialize(JObject jobj)
        {
            ...
            _purchNm = ConvertUtil.ToString(jobj["PURCHNM"]);
        ...
        private void gvList_FocusedRowObjectChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowObjectChangedEventArgs e)
        {
            ...
            if (isValidRow)
            {
                ...
                _ordRate = ConvertUtil.ToInt32(_currentRow["ORDER_RATIO"]);
                _tradeItem = ConvertUtil.ToInt32(_currentRow["TRADE_ITEM"]);
            }
            else
            {
                _currentRow = null;
        ...
        private bool insertData()
        {
            ...
            string inpTime = now.ToString("HHmmSS");
            string ldTime = now.ToString("yyyyMMddHHmmss");
        ...
        public void deleteRowHandler()
        {
            ...
            else
            {
                if (Dangol.MessageYN("선택한 행을 삭제하시겠습니까?") == DialogResult.Yes)
                {
                    ...
                    _currentRow["STORE"] = DBNull.Value;
                    ...
                    SetColFocus("BOOKNM", gvList.FocusedRowHandle);
                }
            }
        }
        ...
        private void gvList_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (_currentRow != null)
            {
                if (e.Column.FieldName != "CHECK")
                {
                    ...
                    else if (e.Column.FieldName == "TRADE_ITEM")
                    {
                        _currentRow["TRADE_ITEM"] = _tradeItem = ConvertUtil.ToInt32(e.Value);
                    }
                }
            }
        }
        ...
        private void insertOrderBook(DataRowView obj)
        {
            ...
            _currentRow["TRADE_ITEM"] = _tradeItem = 1;
        ...
        private void insertOrderBook(DataRow row, JObject obj)
        {
            ...
            row["ORDER_RATIO"] = _ordRate = ConvertUtil.ToInt32(obj["INP_PLAN_RATE"]);
        ...
        private void gvList_CellValueChanging(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            //    if (e.Column.FieldName != "CHECK")
            //    {
            //        if (e.Column.FieldName == "TRADE_ITEM")
            //        {
            //            _currentRow["TRADE_ITEM"] = e.Value;
            //        }
            //    }
            //}
        }
            ...
            if (_dicOrderRatioTable.ContainsKey(key))
            {
                if (init)
                {
                    ...
                    if (dtRatio.Rows.Count > 0)
                    {
                        int rate = 0;
                        _currentRow.BeginEdit();
                        _currentRow["ORDER_RATIO"] = rate = ConvertUtil.ToInt32(dtRatio.Rows[0]["KEY"]);

                        if (rate != _ordRate)
                            _currentRow["ORDER_RATIO"] = _ordRate;
            ...
            if (bookCd > 0 && purchCd > 0)
            {
                ...
                if (DBConnect.getRequest(jobj, ref jResult, url))
                {
                    if (Convert.ToBoolean(jResult["EXIST"]))
                    {
                        ...
                        {
                            _currentRow.BeginEdit();
                            _currentRow["ORDER_RATIO"] = _ordRate = firstRate;
                            _currentRow.EndEdit();
                        }
        ...
        private bool getOrderRatio(DataRow row)
        {
            ...
            if (_dicOrderRatioTable.ContainsKey(key))
            {
                DataTable dtRatio = _dicOrderRatioTable[key];
                if(dtRatio.Rows.Count > 0)
                    row["ORDER_RATIO"] = _ordRate = ConvertUtil.ToInt32(dtRatio.Rows[0]["KEY"]);
            ...
            if (bookCd > 0 && purchCd > 0)
            {
                ...
                if (DBConnect.getRequest(jobj, ref jResult, url))
                {
                    if (Convert.ToBoolean(jResult["EXIST"]))
                    {
                        ...
                        row["ORDER_RATIO"] = _ordRate = firstRate;
        ...
        private void rileOrderRatio_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (popup == null || popup.SelectedIndex < 0)
                {
                    ...
                    SetColFocus("INP_CNT", gvList.FocusedRowHandle);
                }
                else
                {
                    (sender as LookUpEdit).ItemIndex = popup.SelectedIndex;
                    int value = ConvertUtil.ToInt32((sender as LookUpEdit).EditValue);
                    _currentRow["ORDER_RATIO"] = _ordRate = value;
                }
            ...
            {
                if (e.KeyCode != Keys.Space && ConvertUtil.ToInt32(_currentRow["ORDER_RATIO"]) != _ordRate)
                {
                    _currentRow["ORDER_RATIO"] = _ordRate;
                    int price = ConvertUtil.ToInt32(_currentRow["PRICE"]);
                    int orderRatio = _ordRate;
        ...
        private void rileOrderRatio_EditValueChanged(object sender, EventArgs e)
        {
            ...
            _currentRow["ORDER_RATIO"] = _ordRate = ConvertUtil.ToInt32(editor.EditValue);
        ...
        private void rileCondition_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (popup == null && ConvertUtil.ToInt32(_currentRow["TRADE_ITEM"]) == _tradeItem)
                {
        ...
        private void riseInpCnt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                SetColFocus("VCNT", gvList.FocusedRowHandle);
            }
        }
        ...
        private void rileOrderRatio_Popup(object sender, EventArgs e)
        {
            popup = (sender as IPopupControl).PopupWindow as PopupLookUpEditForm;
        }

        private void rileCondition_Popup(object sender, EventArgs e)
        {
            popup = (sender as IPopupControl).PopupWindow as PopupLookUpEditForm;
        }
    }
