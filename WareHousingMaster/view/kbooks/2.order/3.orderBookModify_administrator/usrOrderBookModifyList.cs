...
namespace WareHousingMaster.view.kbooks.search.booksearch
{
    public partial class usrOrderBookModifyList : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void getList(JObject jobj)
        {
            ...
            if (jobj.ContainsKey("STORE_TYPE"))
            {
                ...
                else
                {
                    ...
                    if (end == 99)
                        modifEnd = ProjectInfo._endStore;
        ...
        public void getPerformance()
        {
            ...
            {
                using (dlgBookOutCome bookOutCome = new dlgBookOutCome(ConvertUtil.ToInt32(_jobj["SHOPCD"]), ConvertUtil.ToInt64(_currentRow["BOOKCD"]), 0))
                {
        ...
        public void confirm(int userType)   //0: 관리자, 1:유저
        {
            ...
            else
            {
                if (userType == 1)
                {
                    if (Dangol.MessageYN("주문 데이터를 수정하시겠습니까?") == DialogResult.Yes)
                    {
                        JObject jResult = new JObject();
                        if (save(ref jResult))
                        {
                            Dangol.Info("주문데이터를 수정하였습니다.");
                        }
                        else
                        {
                            Dangol.Error(jResult["MSG"]);
