...
namespace WareHousingMaster.view.kbooks.returns
{
    public partial class usrInputReturnSearch : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void rgProcessType_SelectedIndexChanged(object sender, EventArgs e)
        {
            int type = ConvertUtil.ToInt32(rgProcessType.EditValue);

            if(type == 1)
            {
                rgPurchaseRange.Properties.ReadOnly = true;
                tePurchaseCd.Text = "";
                tePurchaseNm.Text = "";
            }
            else
                rgPurchaseRange.Properties.ReadOnly = false;
        }
