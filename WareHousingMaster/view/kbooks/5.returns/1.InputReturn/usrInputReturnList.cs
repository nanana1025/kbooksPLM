...
{
    public partial class usrInputReturnList : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void setGreidView()
        {
            if (_processType == 1)
            {
                gcPurchCd.OptionsColumn.ReadOnly = true;
                gcProcess.OptionsColumn.ReadOnly = true;
            }
            else
            {
                gcPurchCd.OptionsColumn.ReadOnly = false;
                gcProcess.OptionsColumn.ReadOnly = false;
            }
        ...
        public void setCondition(JObject obj)
        {
            ...
            _processType = ConvertUtil.ToInt32(obj["PROCESS_TYPE"]);
        ...
        public bool insertReturn()
        {
            ...
            else
            {
                if (dataVerification())
                {
                    if (Dangol.MessageYN("반품건을 저장하시겠습니까?") == DialogResult.Yes)
                    {
                        insertData();
                        return true;
                    }
        ...
        public bool isDataClearAvailable()
        {
        ...
        private bool dataVerification()
        {
            ...
            foreach (DataRow row in rows)
            {
                ...
                stock = ConvertUtil.ToInt32(row["STOCK"]);
                returnRate = ConvertUtil.ToInt32(row["RETURN_RATE"]);
                returnCnt = ConvertUtil.ToInt32(row["RETURN_CNT"]);

                if(stock < returnCnt)
                {
                    Dangol.Warining("반품수량이 해당매입처에 재고 수량보다 많은 항목이 존재합니다.");
                    return false;
                }
                else
                {
                    if (bookCd > 0 && purchCd > 0 && returnRate > 0 && returnCnt > 0)
                        row["STATE"] = 2;
                    else
                    {
                        row["STATE"] = 1;
                        faultCnt++;
                        Dangol.Warining("매입처 쪼는 매입률을 확인하세요.");
                        return false;
        ...
        private void insertData()
        {
            ...
            string url = "/returns/insertReturnBook.json";
        ...
        private bool getBookList(long bookCd, string bookNm)
        {
            ...
            if (bookCd == 0)
            {
                if (_processType == 2 && _jobj.ContainsKey("PURCHCD"))
        ...
        private bool setSearchBook(int shopCd, long bookCd, string bookNm, int flag)
        {
            ...
            string url = "/returns/getBookList4Return.json";
                ...
                if (Convert.ToBoolean(jResult["EXIST"]))
                {
                    if (Convert.ToBoolean(jResult["PURCH_EXIST"]))
                    {
                        ...
                        if (rowsBookCd.Length < 1)
                        {
                            ...
                            int minOrder = Convert.ToInt32(dtPurchOrder.AsEnumerable().Min(row => row["KEY"]));

                            _currentRow.BeginEdit();
                            _currentRow["PUR_PROCESS"] = minOrder;
                            setPurchInfo(bookCd, minOrder);
                            _currentRow.EndEdit();
                        ...
                        else
                        {
                            ...
                            int minOrder = Convert.ToInt32(dt.AsEnumerable().Min(row => row["KEY"]));
        ...
        private void gvList_ShownEditor(object sender, EventArgs e)
        {
            ...
            else if (view.FocusedColumn.FieldName == "PURCHCD")
            {
                LookUpEdit editor = (LookUpEdit)view.ActiveEditor;
                long bookCd = ConvertUtil.ToInt64(view.GetFocusedRowCellValue("BOOKCD"));
                if (_dicLookUpEdit.ContainsKey(bookCd))
            ...
            else if (view.FocusedColumn.FieldName == "RETURN_RATE")
            {
                ...
                int currentReturnKbn = ConvertUtil.ToInt32(view.GetFocusedRowCellValue("RETURN_KBN"));

                string key = $"{_shopCd}/{bookCd}/{purchCd}/{currentReturnKbn}";
        ...
        private void rilePurchCd_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            long bookCd = ConvertUtil.ToInt64(_currentRow["BOOKCD"]);
            int purchCd = ConvertUtil.ToInt32(e.NewValue);

            _currentRow.BeginEdit();
            _currentRow["PURCHCD"] = purchCd;
            setPurchOrderInfo(bookCd, purchCd);
            _currentRow.EndEdit();

            int currentReturnKbn = ConvertUtil.ToInt32(_currentRow["RETURN_KBN"]);

            string key = $"{_shopCd}/{_currentRow["BOOKCD"]}/{purchCd}/{currentReturnKbn}";

            _currentRow.BeginEdit();
            //rileOrderRatio.EditValueChanging -= rileOrderRatio_EditValueChanging;
            if (_dicOrderRatioTable.ContainsKey(key))
            {
                int value = ConvertUtil.ToInt32(_dicOrderRatioTable[key].Rows[0]["KEY"]);
                _currentRow["RETURN_RATE"] = value;
            }
            else
                _currentRow["RETURN_RATE"] = -1;
        ...
        private void rileReturnType_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            long bookCd = ConvertUtil.ToInt64(_currentRow["BOOKCD"]);
            int purchCd = ConvertUtil.ToInt32(_currentRow["PURCHCD"]);
            int currentReturnKbn = ConvertUtil.ToInt32(e.NewValue);

            string key = $"{_shopCd}/{bookCd}/{purchCd}/{currentReturnKbn}";

            _currentRow.BeginEdit();
            _currentRow["RETURN_KBN"] = currentReturnKbn;
            rileProcess.EditValueChanging -= rileProcess_EditValueChanging;

            if (_dicOrderRatioTable.ContainsKey(key))
            {
                if (_dicOrderRatioTable[key].Rows.Count > 0)
                {
                    int value = ConvertUtil.ToInt32(_dicOrderRatioTable[key].Rows[0]["KEY"]);
        ...
        private void setPurchInfo(long bookCd, int order)
        {
            int purchCd = -1;
            if (_dicPurchCdTable.ContainsKey(bookCd))
            {
                DataTable dt = _dicPurchCdTable[bookCd];

                DataRow[] rows = dt.Select($"ORDER = {order}");

                if (rows.Length > 0)
                {
                    rilePurchCd.EditValueChanging -= rilePurchCd_EditValueChanging;
                    _currentRow["PURCHCD"] = rows[0]["KEY"];
                    _currentRow["PURCHNM"] = rows[0]["VALUE"];
                    rilePurchCd.EditValueChanging += rilePurchCd_EditValueChanging;
                    purchCd = ConvertUtil.ToInt32(rows[0]["KEY"]);
                }
            }

            setETCInfo(_shopCd, bookCd, purchCd);
        }

        private void setPurchOrderInfo(long bookCd, int purchCd)
        {
            if (_dicPurchCdTable.ContainsKey(bookCd))
            {
                DataTable dt = _dicPurchCdTable[bookCd];

                DataRow[] rows = dt.Select($"KEY = {purchCd}");

                if (rows.Length > 0)
                {
                    rileProcess.EditValueChanging -= rileProcess_EditValueChanging;
                    _currentRow["PUR_PROCESS"] = rows[0]["ORDER"];
                    _currentRow["PURCHNM"] = rows[0]["VALUE"];
                    rileProcess.EditValueChanging += rileProcess_EditValueChanging;
                }
            }
        ...
        private void setETCInfo(int shopCd, long bookCd, int purchCd)
        {
            foreach (KeyValuePair<int, string> item in Common._dicRateKbn)
            {
                _currentRow[$"RATE{item.Key}"] = DBNull.Value;
            ...
            if (purchCd > 0)
            {
                ...
                if (DBConnect.getRequest(jobj, ref jResult, url))
                {
                    _currentRow["RET_PLAN_CNT"] = ConvertUtil.ToInt32(jResult["RETURN_PLAN_CNT"]);
                    ...
                    if (Convert.ToBoolean(jResult["EXIST"]))
                    {
                        ...
                        DataTable dtOrderRatioTable2;
                        dtOrderRatioTable2 = new DataTable();
                        dtOrderRatioTable2.Columns.Add(new DataColumn("KEY", typeof(int)));
                        dtOrderRatioTable2.Columns.Add(new DataColumn("VALUE", typeof(string)));
                        dtOrderRatioTable2.Columns.Add(new DataColumn("RATE_KBN", typeof(int)));
                        ...
                        foreach (JObject obj in jArray.Children<JObject>())
                        {
                            ...
                            {
                                _currentRow[$"RATE{rateKbn}"] = rate;
                                _currentRow[$"STOCK{rateKbn}"] = ConvertUtil.ToInt32(obj["STOCK"]);
                            }
                            ...
                            {
                                DataRow dr1 = dtOrderRatioTable2.NewRow();

                                dr1["KEY"] = rate;
                                dr1["VALUE"] = ConvertUtil.ToString(obj["RATE"]);
                                dr1["RATE_KBN"] = rateKbn;

                                dtOrderRatioTable2.Rows.Add(dr1);

                                if (index == 1 && currentReturnKbn == 2)
                                {
                                    firstRateKbn = rateKbn;
                                    firstRate = rate;
                                }
                        ...
                        if(!_dicOrderRatioTable.ContainsKey($"{key}/1"))
                            _dicOrderRatioTable.Add($"{key}/1", dtOrderRatioTable);
                        if (!_dicOrderRatioTable.ContainsKey($"{key}/2"))
                            _dicOrderRatioTable.Add($"{key}/2", dtOrderRatioTable2);

                        _currentRow["RETURN_RATE"] = firstRate;
                        _currentRow["RATE_KBN"] = firstRateKbn;
                    }
                }
                else
                {
        ...
        private void riseCnt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                int stock = ConvertUtil.ToInt32(_currentRow["STOCK"]);
                int returnCnt = ConvertUtil.ToInt32(_currentRow["RETURN_CNT"]);
                ...
                if(stock < returnCnt)
                {
                    ...
                    Dangol.Warining("입력하신 반품수량이 해당매입처에 재고 수량보다 많습니다.\n 다른 매입처로 반품을 권장합니다.");
                }
                else
                {
                    int rowHandle = gvList.FocusedRowHandle;
        ...
        private void riseCnt_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            _currentRow.BeginEdit();
            _currentRow["RETURN_CNT"] = ConvertUtil.ToInt32(e.NewValue);
            _currentRow.EndEdit();
        }
    }
}
