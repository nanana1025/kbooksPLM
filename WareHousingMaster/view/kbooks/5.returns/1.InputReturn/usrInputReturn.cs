...
namespace WareHousingMaster.view.kbooks.returns
{
    public partial class usrInputReturn : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void usrForm_Load(object sender, EventArgs e)
        {
            ...
            usrInputReturnSearch1.clearHandler += new usrInputReturnSearch.ClearHandler(clear);
        ...
        private void reset(bool isCheck = true)
        {
            if(isCheck)
            {
                if (usrInputReturnList1.isDataClearAvailable())
                {
        ...
        {
            if (usrInputReturnList1.insertReturn())
            {
                reset(false);
            }
        }
