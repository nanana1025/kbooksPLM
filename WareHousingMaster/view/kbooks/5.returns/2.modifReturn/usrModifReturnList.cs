...
namespace WareHousingMaster.view.kbooks.returns
{
    public partial class usrModifReturnList : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public usrModifReturnList()
        {
            ...
            _dt.Columns.Add(new DataColumn("PUR_PROCESS", typeof(int)));
        ...
        public void setInitLoad(int viewType = 1) //1:수정/출력, 2:관리자
        {
            setInfoBox();
            setinitialize();
            _viewType = viewType;
        ...
        public bool insertReturn()
        {
            ...
            else
            {
                ...
                if (dataVerification())
                {
                    ...
                    if (_viewType == 1)
                    {
                        if (Dangol.MessageYN(msg) == DialogResult.Yes) //save
                        {
            ...
            if (DBConnect.getRequest(_jobj, ref jResult, url))
            {
                if (Convert.ToBoolean(jResult["EXIST"]))
                {
                    ...
                    foreach (JObject obj in jArray.Children<JObject>())
                    {
                        ...
                        //dr["PUR_PROCESS"] = ConvertUtil.ToInt32(obj["STOCK"]);
                        ...
                        dr["FIRSTSTORE"] = ConvertUtil.ToDateTimeNull(obj["FIRSTSTORE"]);
        ...
        private void riseCnt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                else
                {
                    int rowHandle = gvList.FocusedRowHandle;
                    if (rowHandle < 29)
                        SetColFocus("RETURN_CNT", rowHandle + 1);
                }
