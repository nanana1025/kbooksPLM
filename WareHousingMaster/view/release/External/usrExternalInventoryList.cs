...
{
    public partial class usrExternalInventoryList : DevExpress.XtraEditors.XtraForm
    {
        ...
        public usrExternalInventoryList()
        {
            InitializeComponent();

            _dtProduce = new DataTable();
            _dtProduce.Columns.Add(new DataColumn("NO", typeof(long)));
            _dtProduce.Columns.Add(new DataColumn("PRODUCE_ID", typeof(long)));
            _dtProduce.Columns.Add(new DataColumn("COMPANY_ID", typeof(long)));
            _dtProduce.Columns.Add(new DataColumn("PRODUCE", typeof(string)));
            _dtProduce.Columns.Add(new DataColumn("STATE", typeof(int)));
            //_dtProduce.Columns.Add(new DataColumn("RELEASE_DT", typeof(string)));
            //_dtProduce.Columns.Add(new DataColumn("ARRIVAL_DT", typeof(string)));
            _dtProduce.Columns.Add(new DataColumn("PRODUCT_CNT", typeof(int)));
            _dtProduce.Columns.Add(new DataColumn("PART_CNT", typeof(int)));
            _dtProduce.Columns.Add(new DataColumn("RELEASE_CNT", typeof(int)));
            _dtProduce.Columns.Add(new DataColumn("PRICE", typeof(long)));
            ...
            _dt.Columns.Add(new DataColumn("PRODUCE_ID", typeof(long)));
            ...
            _dt.Columns.Add(new DataColumn("OS", typeof(int)));
        ...
        private void setInfoBox()
        {
            ...
            Util.insertRowonTop(dtWarehousingState, 1, "comp");
            ...
            dtState.Columns.Add(new DataColumn("KEY", typeof(int)));
            dtState.Columns.Add(new DataColumn("VALUE", typeof(string)));
            Util.insertRowonTop(dtState, 0, "");
            Util.insertRowonTop(dtState, 1, "proc");
            Util.insertRowonTop(dtState, 2, "comp");

            Util.LookupEditHelper(rileState, dtState, "KEY", "VALUE");
        ...
        private void setIInitData()
        {
            ...
            _bsExport.DataSource = _dtProduce;
        ...
        private void gvExport_FocusedRowObjectChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowObjectChangedEventArgs e)
        {
            ...
            if (isValidRow)
            {
                ...
                _produceId = ConvertUtil.ToInt64(_currentExportRow["PRODUCE_ID"]);
        ...
        private bool getExportList()
        {
            ...
            if (DBRelease.getInventoryManagementList(jData, ref jResult))
            {
                if (Convert.ToBoolean(jResult["EXIST"]))
                {
                    ...
                    foreach (JObject obj in jArray.Children<JObject>())
                    {
                        ...
                        dr["NO"] = index++;
                        dr["PRODUCE_ID"] = obj["PRODUCE_ID"];
                        dr["STATE"] = ConvertUtil.ToInt32(obj["STATE"]);
        ...
        private void geList()
        {
            ...
            jData.Add("PRODUCE_ID", _produceId);

            _dt.Clear();
            _dtProductDetail.Clear();

            if (DBRelease.getInventoryList(jData, ref jResult))
            {
                int index = 1;
                ...
                if (Convert.ToBoolean(jResult["PRODUCT_EXIST"]))
                {
                    ...
                    foreach (JObject obj in jArray.Children<JObject>())
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(ConvertUtil.ToString(obj["NEW_COA"])))
                            dr["OS"] = 1;
                        else
                            dr["OS"] = 0;
        ...
        private bool checkSearch(ref JObject jData)
        {
            ...
            jData.Add("PRODUCE_DT_S", dtFrom);
            jData.Add("PRODUCE_DT_E", dtTo);
        ...
        private void lcgBarcodeList_CustomButtonClick(object sender, DevExpress.XtraBars.Docking2010.BaseButtonEventArgs e)
        {
            ...
            if (e.Button.Properties.Tag.Equals(1))
            {
                ...
                if (Dangol.MessageYN("Save the changes?") == DialogResult.Yes)
                {
                    ...
                    foreach (DataRow row in rows)
                    {
                        ...
                        jdata.Add("EXPORT_ID", ConvertUtil.ToInt32(row["EXPORT_ID"]));
                        jArray.Add(jdata);                        
                    }

                    jobj.Add("DATA", jArray);
                    jobj.Add("PRODUCE_ID", _produceId);

                    if (DBRelease.createInventoryInfo(jobj, ref jResult))
                    {
            ...
            else if (e.Button.Properties.Tag.Equals(2))
            {
                ...
                if (rows.Length < 1)
                {
                    Dangol.Message("Please choose the management number");
                    return;
                }

                if (Dangol.MessageYN("Delete the selected management number from the list?") == DialogResult.Yes)
                {
                    Dangol.ShowSplash();
                    ...
                    if (DBRelease.deleteInventoryInfo(jobj, ref jResult))
                    {
                        gvList.BeginDataUpdate();
                        ...
                        {
                            inventoryId = ConvertUtil.ToInt64(row["INVENTORY_ID"]);
                            rowsDetail = _dtProductDetail.Select($"INVENTORY_ID = {inventoryId}");
        ...
        private void layoutControlGroup1_CustomButtonClick(object sender, DevExpress.XtraBars.Docking2010.BaseButtonEventArgs e)
        {
            ...
            if (e.Button.Properties.Tag.Equals(1))
            {
                ...
                if (rows.Length < 1)
                {
                    Dangol.Message("출고번호를 선택하세요");
                    return;
                }

                if (Dangol.MessageYN("선택하신 출고번호의 제품 정보를 확인하시겠습니까?") == DialogResult.No)
                {
                    return;
                }
            ...
            else if (e.Button.Properties.Tag.Equals(3))
            {
                ...
                if (Dangol.MessageYN("Create a regist number?") == DialogResult.No)
                {
                    return;
                }
                ...
                if (DBRelease.createInventoryProduce(jData, ref jResult))
                {
                }
            ...
            {
                if(_currentExportRow == null)
                {
                    Dangol.Message("There are no selected regist number.");
                    return;
                }

                if (Dangol.MessageYN("Delete the selected regist number?") == DialogResult.No)
                {
                    return;
                }
                ...
                if (DBRelease.updateInventoryProduce(jData, ref jResult))
                {
                    _currentExportRow.Delete();
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < gvExport.DataRowCount; i++)
                        {
                            ...
                            arows.Add(gvExport.GetDataRow(rowHandle));
                        }
            ...
            else if (e.Button.Properties.Tag.Equals(5))
            {
                ...
                if (Dangol.MessageYN("Complete the registration number?") == DialogResult.No)
                {
                    return;
                }
            ...
            else if (e.Button.Properties.Tag.Equals(6))
            {
                ...
                if (Dangol.MessageYN("Change the regist number to in progress?") == DialogResult.No)
                {
                    return;
                }
        ...
        private void sbInput_Click(object sender, EventArgs e)
        {
            if(_produceId < 0)
            {
                Dangol.Message("Please select the regist number.");
                return;
            }
                    ...
                    if (DBRelease.getExportDataOneDetail(jData, ref jResult))
                    {
                        ...
                        if (Convert.ToBoolean(jResult["PRODUCT_EXIST"]))
                        {
                            ...
                            foreach (JObject obj in jArray.Children<JObject>())
                            {
                                ...
                                //dr["PRODUCE"] = obj["PRODUCE"];
                                dr["EXPORT_ID"] = obj["EXPORT_ID"];
                                dr["PRODUCE_ID"] = _produceId;
                                dr["PRODUCE"] = _currentExportRow["PRODUCE"];
                                dr["BARCODE"] = obj["BARCODE"];
                                dr["PRICE"] = obj["PRICE"];
                                dr["COMPONENT_CD"] = obj["COMPONENT_CD"];
                                //dr["RELEASE_DT"] = ConvertUtil.ToDateTimeNull(obj["RELEASE_DT"]);
