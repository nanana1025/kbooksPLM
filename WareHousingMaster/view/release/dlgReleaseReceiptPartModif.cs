...
{
    public partial class dlgReleaseReceiptPartModif : DevExpress.XtraEditors.XtraForm
    {
        ...
        //Dictionary<long, DataTable> _dicReceiptPartDetail;
        List<long> _listReceiptInventoryId;
        List<long> _listReceiptProductId;
        ...
        Dictionary<int, string> dicExportComponentCd;
        Dictionary<string, int> dicExportComponentCdReverse;
        ...
        public dlgReleaseReceiptPartModif(long exportId)
        {
            InitializeComponent();
            ...
            _dicPartDetail = new Dictionary<long, DataTable>();
            //_dicReceiptPartDetail = new Dictionary<long, DataTable>();

            _listReceiptInventoryId = new List<long>();
            _listReceiptProductId = new List<long>();
            ...
            dicExportComponentCd = new Dictionary<int, string>();
            dicExportComponentCdReverse = new Dictionary<string, int>();

            for (int i = 0; i < _exportComponetCd.Length; i++)
            {
                dicExportComponentCd.Add(i, _exportComponetCd[i]);
                dicExportComponentCdReverse.Add(_exportComponetCd[i], i);
            }
        ...
        private void dlgReleaseReceiptPartModif_Load(object sender, EventArgs e)
        {
            setInfoBox();
            setIInitData();
            setGridControl();
        ...
        private void getInitData(bool isInit)
        {
            JObject jResult = new JObject();
            JObject jobj = new JObject();
            ...
            if (DBRelease.getExportPartInfo(jobj, ref jResult))
            {
                gvPart.FocusedRowObjectChanged -= gvPart_FocusedRowObjectChanged;
                gvPartDetail.FocusedRowObjectChanged -= gvComponentList_FocusedRowObjectChanged;

                addPart(jResult, isInit);
        ...
        private bool addPart(JObject jResult, bool isInit = false)
        {
            ...
            if (Convert.ToBoolean(jResult["PRODUCT_EXIST"]))
            {
                ...
                foreach (JObject obj in jArray.Children<JObject>())
                {
                    ...
                    if (!_listReceiptInventoryId.Contains(inventoryId))
                    {
                        ...
                        if (ConvertUtil.ToString(obj["LOCK_YN"]).Equals("Y") && !isInit)
                            dr["ETC"] = "사용중";
                        ...
                        _listReceiptInventoryId.Add(inventoryId);
            ...
            if (Convert.ToBoolean(jResult["PART_EXIST"]))
            {
                ...
                foreach (JObject obj in jArray.Children<JObject>())
                {
                    ...
                    if (!_listReceiptInventoryId.Contains(inventoryId))
                    {
                        ...
                        _listReceiptInventoryId.Add(inventoryId);
                        _listReceiptProductId.Add(inventoryId);
        ...
        private void receipt(DataRow rowReceipt, long id)
        {
            //JObject jResult = new JObject();
            //JObject jobj = new JObject();
            //var jArray = new JArray();
        ...
        private void DeleteBarcode()
        {
            ...
            foreach (DataRow row in rows)
            {
                ...
                _listReceiptInventoryId.Remove(inventoryId);
                _listReceiptProductId.Remove(inventoryId);
                row.Delete();
            }
            _dtPart.EndInit();
        ...
        {
            if (Dangol.MessageYN("부품/제품 정보를 수정하시겠습니까?") == DialogResult.No)
            {
                return;
            }
            ...
            //DataRow[] rows = dt.Select("PRODUCT_STATE < 3 AND LOCK_YN = 'N'");
            DataRow[] rows = _dtPart.Select("(PRODUCT_YN = 1 AND PRODUCT_STATE = 3) OR (PRODUCT_YN = 0 AND INVENTORY_STATE  = 'E')");
            DataRow[] rowsDetail;
            long pInventoryId;
            int productYn;
            ...
            foreach (DataRow row in rows)
            {
                ...
                if (!productInventoryId.Contains(pInventoryId) && productYn == 1)
                {
                    ...
                    dtPartDetail = _dicPartDetail[pInventoryId];
            ...
            if (DBRelease.updateReceiptPart(jobj, ref jResult))
            {
