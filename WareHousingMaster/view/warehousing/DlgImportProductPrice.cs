...
{
    public partial class DlgImportProductPrice : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool updatePrice(DataTable dt)
        {
            ...
            try
            {
                ...
                {
                    string barcode = ConvertUtil.ToString(row["관리번호"]);
                    long price = ConvertUtil.ToInt64(row["실매입가"]);
                ...
                if (DBConnect.updateProductPriceBulk(listBarcode, listPrice, ref jResult))
                {
