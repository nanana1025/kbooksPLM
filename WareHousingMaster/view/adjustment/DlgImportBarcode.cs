...
namespace WareHousingMaster.view.adjustment
{
    public partial class DlgImportBarcode : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void Standard(object sender, DoWorkEventArgs e)
        {
            ...
            tlResult.Invoke(new Action(() => tlResult.Nodes.Add(new object[] { ++_errCount, $"불러온 데이터를 기반으로 재고번호를 구성합니다.", (int)EnumInfoCode.Info })));
            backgroundWorker1.ReportProgress(50);

            if (!AddReceiptByExcel1(_dtExcel))
            {
                tlResult.Invoke(new Action(() => tlResult.Nodes.Add(new object[] { ++_errCount, $"재고번호 구성을 실패하였습니다.", (int)EnumInfoCode.Failure })));
                backgroundWorker1.ReportProgress(100);
                return;
            }
            backgroundWorker1.ReportProgress(80);
            tlResult.Invoke(new Action(() => tlResult.Nodes.Add(new object[] { ++_errCount, $"재고번호 구성을 완료했습니다.", (int)EnumInfoCode.Info })));
            backgroundWorker1.ReportProgress(100);
